...
func TestDateCheckFilter(t *testing.T) {
	...
	require.NotNil(t, f)
	require.Equal(t, "divendres", f.GetDayOfWeekName(2014, 8, 29))
	require.Equal(t, "dissabte", f.GetDayOfWeekName(2014, 8, 23))
}

func TestDateCheckFilter_AcceptWrongWeekday(t *testing.T) {
	// 2014-08-23 is Saturday (dissabte); claiming diumenge keeps the match
	f := NewDateCheckFilter()
	m := rules.NewRuleMatch(rules.NewFakeRule("D"), nil, 0, 10, "wrong {realDay} not {day}")
	out := f.AcceptRuleMatch(m, map[string]string{
		"year": "2014", "month": "8", "day": "23", "weekDay": "diumenge",
	}, 0, nil, nil)
	require.NotNil(t, out)
	require.Contains(t, out.GetMessage(), "dissabte")
}
...
func TestDateCheckFilter_AcceptCorrectWeekday(t *testing.T) {
	...
	out := f.AcceptRuleMatch(m, map[string]string{
		"year": "2014", "month": "8", "day": "23", "weekDay": "dissabte",
	}, 0, nil, nil)
	require.Nil(t, out)
}
...
func TestDateCheckFilter_Registered(t *testing.T) {
	require.True(t, patterns.GlobalRuleFilterCreator.HasFilter("org.languagetool.rules.ca.DateCheckFilter"))
}
