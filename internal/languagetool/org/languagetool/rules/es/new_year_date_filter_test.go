...
func TestNewYearDateFilter_AcceptRuleMatch(t *testing.T) {
	...
	out = f.AcceptRuleMatch(m, map[string]string{
		"year": "2013", "month": "marzo", "day": "15",
	}, 0, nil, nil)
	require.NotNil(t, out)
}

func TestNewYearDateFilter_Registered(t *testing.T) {
	require.True(t, patterns.GlobalRuleFilterCreator.HasFilter("org.languagetool.rules.es.NewYearDateFilter"))
}
