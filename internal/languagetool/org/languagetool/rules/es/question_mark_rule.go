...
func NewQuestionMarkRule(messages map[string]string) *QuestionMarkRule {
	r := &QuestionMarkRule{Messages: messages}
	// Java: Qué pasa? → ¿Qué pasa?
	r.AddExamplePair(
		rules.Wrong("<marker>Qué</marker> pasa?"),
		rules.Fixed("<marker>¿Qué</marker> pasa?"),
	)
	return r
}
...
func (r *QuestionMarkRule) AddExamplePair(incorrect rules.IncorrectExample, correct rules.CorrectExample) {
	if r == nil {
		return
	}
...
func (r *QuestionMarkRule) GetIncorrectExamples() []rules.IncorrectExample {
...
func (r *QuestionMarkRule) GetCorrectExamples() []rules.CorrectExample {
