...
// DateCheckFilter ports org.languagetool.rules.es.DateCheckFilter
// (extends AbstractDateCheckWithSuggestionsFilter with ES localization).
...
func NewDateCheckFilter() *DateCheckFilter {
	return &DateCheckFilter{
		...
		inner:  esDateCheckWithSuggestions(),
	}
}
...
func (f *DateCheckFilter) GetDayOfWeekName(year, month, day int) string {
	...
	return formatESDayOfWeek(t)
}

func formatESDayOfWeek(t time.Time) string {
...
func (f *DateCheckFilter) AcceptRuleMatch(match *rules.RuleMatch, arguments map[string]string, _ int,
	...
	if f.inner == nil {
		f.inner = esDateCheckWithSuggestions()
	}
