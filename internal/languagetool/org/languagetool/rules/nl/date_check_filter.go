...
// DateCheckFilter ports org.languagetool.rules.nl.DateCheckFilter
// (extends AbstractDateCheckWithSuggestionsFilter with NL localization).
...
func NewDateCheckFilter() *DateCheckFilter {
	return &DateCheckFilter{
		...
		inner:  nlDateCheckWithSuggestions(),
	}
}
...
func (f *DateCheckFilter) GetDayOfWeekName(year, month, day int) string {
	...
	return formatNLDayOfWeek(t)
}

func formatNLDayOfWeek(t time.Time) string {
...
func (f *DateCheckFilter) AcceptRuleMatch(match *rules.RuleMatch, arguments map[string]string, _ int,
	...
	if f.inner == nil {
		f.inner = nlDateCheckWithSuggestions()
	}
