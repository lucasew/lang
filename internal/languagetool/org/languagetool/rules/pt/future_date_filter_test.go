...
func TestFutureDateFilter_AcceptRuleMatch(t *testing.T) {
	...
	// localized Portuguese month
	out = f.AcceptRuleMatch(m, map[string]string{
		"year": "2015", "month": "junho", "day": "15",
	}, 0, nil, nil)
	require.NotNil(t, out)
	require.Nil(t, f.AcceptRuleMatch(m, map[string]string{
...
func TestFutureDateFilter_Registered(t *testing.T) {
	require.True(t, patterns.GlobalRuleFilterCreator.HasFilter("org.languagetool.rules.pt.FutureDateFilter"))
}
