...
func TestYMDNewYearDateFilter_ShouldFlag(t *testing.T) {
	f := NewYMDNewYearDateFilter()
	jan := true
	y := 2014
	f.newYear.ForceJanuary = &jan
	f.newYear.ForceYear = &y
	ok, err := f.ShouldFlagFromArgs(map[string]string{"date": "2013-03-15", "weekDay": "sexta"})
...
func TestYMDNewYearDateFilter_AcceptRuleMatch(t *testing.T) {
	...
	out = f.AcceptRuleMatch(m, map[string]string{"date": "2013-12-01", "weekDay": "dom"}, 0, nil, nil)
	require.Nil(t, out)
}
...
func TestYMDNewYearDateFilter_RejectsYearKey(t *testing.T) {
	...
	require.Panics(t, func() {
		f.AcceptRuleMatch(m, map[string]string{"date": "2013-03-15", "year": "2013", "weekDay": "sexta"}, 0, nil, nil)
	})
}

func TestYMDNewYearDateFilter_Registered(t *testing.T) {
	require.True(t, patterns.GlobalRuleFilterCreator.HasFilter("org.languagetool.rules.pt.YMDNewYearDateFilter"))
}
