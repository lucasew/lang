...
func isSoftFalseFriendGroupID(id string) bool {
	...
	case "ABILITY", "GIFT", "ACTUAL", "LIBRARY", "EVENTUAL", "BECOME",
		...
		"GENIAL", "OVERTAKE", "FABRICATE", "GENEROUS", "NORMAL", "CASUAL",
		"PRETEND", "SUPPORT", "ATTEND", "SUCCESS", "LARGE", "SANE":
		return true
	default:
		return false
	}
...
func softRegionalTypoID(id string) bool {
	for _, suf := range []string{"_US", "_GB", "_BR", "_PT", "_MX", "_ES", "_CH", "_AT", "_CA", "_AU", "_NZ"} {
...
func softStyleID(id string) bool {
	if strings.Contains(id, "_STYLE") || strings.Contains(id, "KIND_OF") ||
		...
		strings.Contains(id, "PICKY_") || strings.Contains(id, "SOFT_OPT_") ||
		strings.Contains(id, "_OPT_") {
		return true
	}
	return false
}
