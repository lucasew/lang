...
const PersianDictPath = "/fa/fa.dict"

type PersianTagger struct { *tagging.BaseTagger }

func NewPersianTagger(wt tagging.WordTagger) *PersianTagger {
	return &PersianTagger{BaseTagger: tagging.NewBaseTagger(wt, PersianDictPath, "fa", false)}
}

func (t *PersianTagger) Tag(sentenceTokens []string) []*languagetool.AnalyzedTokenReadings {
	if t == nil { return nil }
