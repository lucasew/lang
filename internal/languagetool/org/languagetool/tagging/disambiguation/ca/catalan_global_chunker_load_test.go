...
func TestOpenCatalanGlobalChunker_SettingsAndNoIgnore(t *testing.T) {
	// Tiny in-memory stand-in of official format (not invent for engine input —
	// proves OpenCatalanGlobalChunker applies Java constructor settings).
	r := strings.NewReader("Foo Bar\n")
	c, err := OpenCatalanGlobalChunker(r)
	require.NoError(t, err)
	require.NotNil(t, c)
	require.False(t, c.AddIgnoreSpelling, "Catalan GlobalChunker does NOT setIgnoreSpelling")
	require.False(t, c.RemovePreviousTags, "Catalan GlobalChunker does NOT setRemovePreviousTags")
	...
	d := &CatalanHybridDisambiguator{GlobalChunker: c}
...
func TestCatalanGlobalChunker_ProcessCachedOfficial(t *testing.T) {
	if DiscoverCatalanGlobalChunker() == "" {
		t.Skip("official spelling_global.txt not discoverable")
	}
	a := CatalanGlobalChunker()
	b := CatalanGlobalChunker()
	require.NotNil(t, a)
...
func TestCatalanGlobalChunker_DisambiguateOfficialPhrases(t *testing.T) {
	if DiscoverCatalanGlobalChunker() == "" {
		t.Skip("official spelling_global.txt not discoverable")
	}
	...
	require.NotNil(t, g)
	d := &CatalanHybridDisambiguator{GlobalChunker: g}
	...
	for _, tc := range cases {
		...
		// Never ignore spelling from CA GlobalChunker
		for i, tr := range out.GetTokens() {
			if i == 0 || tr.IsWhitespace() {
				continue
			}
	...
	// NewCatalanHybridDisambiguator wires GlobalChunker first; multiwords Chunker then
