...
func TestDebugITMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "it")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "it")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "it"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("it full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
