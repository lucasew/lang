...
func TestDebugTLMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "tl")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "tl")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "tl"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("tl full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
