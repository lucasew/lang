...
func TestDebugELMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "el")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "el")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "el"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("el full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
