...
func TestDebugSKMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "sk")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "sk")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "sk"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("sk full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
