...
func TestDebugRUMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "ru")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "ru")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "ru"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("ru full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 40 {
			break
		}
