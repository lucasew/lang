...
func TestDebugISMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "is")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "is")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "is"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("is full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
