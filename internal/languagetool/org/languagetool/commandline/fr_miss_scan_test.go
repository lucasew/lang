...
func TestDebugFRMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "fr")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "fr")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "fr"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("fr full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, 100*float64(passed)/float64(tried))
	for i, x := range ks {
		if i >= 30 {
			break
		}
