...
func TestDebugGAMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "ga")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "ga")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "ga"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("ga full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
