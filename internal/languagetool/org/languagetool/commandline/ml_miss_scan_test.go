...
func TestDebugMLMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "ml")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "ml")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "ml"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("ml full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
