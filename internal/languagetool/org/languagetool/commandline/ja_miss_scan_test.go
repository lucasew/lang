...
func TestDebugJAMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "ja")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "ja")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "ja"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("ja full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
