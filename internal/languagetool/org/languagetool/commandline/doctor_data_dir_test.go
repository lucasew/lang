...
func TestCoreDoctor(t *testing.T) {
	...
	if strings.Contains(out, "grammar dir: (unset)") {
		...
		if strings.Contains(out, "es-MX-soft.xml") {
			require.Contains(t, out, "es-MX soft smoke:")
		}
		if strings.Contains(out, "de-CH-soft.xml") {
			require.Contains(t, out, "de-CH soft smoke:")
		}
		if strings.Contains(out, "fr-CA-soft.xml") {
			require.Contains(t, out, "fr-CA soft smoke:")
		}
		...
		if strings.Contains(out, "en-picky-soft.xml") || strings.Contains(out, "picky soft pack:") {
			require.Contains(t, out, "picky soft pack:")
			require.Contains(t, out, "--level picky")
			require.Contains(t, out, "picky soft packs:")
		}
	}
}
