...
func TestDebugARMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "ar")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "ar")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "ar"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("ar full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
