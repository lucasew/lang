...
func TestDebugFAMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "fa")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "fa")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "fa"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("fa full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
