...
func TestDebugEOMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "eo")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "eo")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "eo"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("eo full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
