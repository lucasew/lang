...
func TestDebugBEMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "be")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "be")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "be"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("be full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
