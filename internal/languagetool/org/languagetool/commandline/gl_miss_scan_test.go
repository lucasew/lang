...
func TestDebugGLMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "gl")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "gl")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "gl"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("gl full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 40 {
			break
		}
