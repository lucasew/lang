...
func TestDebugROMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "ro")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "ro")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "ro"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("ro full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 40 {
			break
		}
