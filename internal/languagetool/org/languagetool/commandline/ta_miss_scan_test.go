...
func TestDebugTAMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "ta")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "ta")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "ta"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("ta full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
