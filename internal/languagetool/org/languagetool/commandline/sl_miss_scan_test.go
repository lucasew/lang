...
func TestDebugSLMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "sl")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "sl")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "sl"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("sl full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
