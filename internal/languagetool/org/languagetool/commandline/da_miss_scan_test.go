...
func TestDebugDAMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "da")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "da")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "da"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("da full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
