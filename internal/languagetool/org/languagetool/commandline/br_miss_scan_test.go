...
func TestDebugBRMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "br")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "br")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "br"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("br full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
