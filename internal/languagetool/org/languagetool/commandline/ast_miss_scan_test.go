...
func TestDebugASTMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "ast")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "ast")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "ast"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("ast full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
