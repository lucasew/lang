...
func TestDebugKMMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "km")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "km")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "km"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("km full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
