...
func TestDebugESMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "es")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "es")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "es"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("es full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, 100*float64(passed)/float64(tried))
	for i, x := range ks {
		if i >= 30 {
			break
		}
