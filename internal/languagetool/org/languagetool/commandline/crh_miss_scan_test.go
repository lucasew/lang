...
func TestDebugCRHMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "crh")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "crh")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "crh"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("crh full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
