...
func TestDebugSVMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "sv")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "sv")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "sv"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("sv full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
