...
func TestDebugNLMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "nl")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "nl")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "nl"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("nl full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
