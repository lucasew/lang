...
func TestDebugPTMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "pt")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "pt")
	...
	for _, tc := range doc.Cases {
		...
		_, err := CoreGoldenHook(&buf, tc.Text, &CommandLineOptions{Language: "pt"})
		if err != nil {
			byRule[tc.Rule]++
			continue
		}
	...
	t.Logf("pt full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, pct)
	for i, x := range ks {
		if i >= 30 {
			break
		}
