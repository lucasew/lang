...
func TestDebugDEMissScan(t *testing.T) {
	...
	doc := loadUpstreamGoldens(t, "de")
	optionalIDs := loadOptionalUpstreamSoftRuleIDs(t, "de")
	...
	t.Logf("de full: passed=%d missed=%d of %d (%.1f%%)", passed, tried-passed, tried, 100*float64(passed)/float64(tried))
	for i, x := range ks {
		if i >= 30 {
			break
		}
